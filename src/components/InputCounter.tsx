...
const InputCounter = (props: Props) => {
  ...
  return (
    <div className='border-[1px] border-gray-300 inline-block rounded-full'>
      <button className='px-3 py-1 text-base lg:text-xl rounded-full hover:bg-gray-100'
        onClick={handleDecreaseItem}>{quantity === 1 ? <TrashIcon className='size-3 lg:size-4 hover:text-red-500'/> : '-'}</button>
      <input type="text" disabled value={quantity} className='bg-white px-2 w-10 text-center text-sm lg:text-base'/>
      <button className='px-3 py-1 text-base lg:text-xl rounded-full hover:bg-gray-100' 
        onClick={handleIncreseItem}>+</button>
    </div>
  )
}
