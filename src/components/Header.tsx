...
const Header = () => {
  ...
  return (
    <div className='flex gap-x-2 items-center justify-between bg-slate-900 px-4 -mx-4 sm:px-6 sm:-mx-6 md:px-10 md:-mx-10 lg:-mx-20 lg:px-20 py-3'>
      ...
      <div className='flex-grow xs:flex-grow-0 xs:min-w-[200px] md:min-w-[360px] relative' ref={popupRef}>
        <input type="text" className='border-none outline-none px-3 py-2 rounded-sm text-xs md:text-sm w-full' 
      ...
      <div className='relative cursor-pointer' onClick={() => navigate('/cart')}>
        ...
        {state.products.length > 0 && 
          <span className='absolute -top-1/4 -right-1/4 size-5 rounded-full bg-red-600 text-center text-white text-sm'>{state.products.length}</span>
        }
      </div>
    </div>
  )
