    ...
    <div className='grid grid-cols-12 gap-x-6 2xl:gap-x-10 gap-y-2 md:gap-y-0'>
      <div className='col-span-full md:col-span-9'>
        ...
        {state.products.length > 0 ? (
          state.products.map((product) => <CartItem product={product} key={product.id} />)
        ) : (
      ...
      <div className='col-span-full md:col-span-3'>
        ...
        <div className='w-full border-[1px] border-gray-200 my-2'></div>
        <button className='w-full rounded-3xl py-2 bg-black text-white hover:bg-gray-900 my-4'>Checkout</button>
      </div>
    </div>
  );
};
