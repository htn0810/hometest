...
const Products = () => {
  ...
  const handleSort = (sort: ISort) => {
    setFilters((prev) => {
      if (prev.sort && prev.sort.key === sort.key && prev.sort.type === sort.type) {
  ...
  return (
    <div className='relative'>
      <div className='flex justify-end gap-x-4'>
        <div className='flex gap-x-2 items-center cursor-pointer' onClick={() => setShowFilters((prev) => !prev)}>
        ...
        <Dropdown title='Sort By' icon={<ArrowDownIcon className='size-6' />}>
          <DropdownItem
            ...
            isActive={filters.sort?.key === 'price' && filters.sort.type === 'desc'}
          >
            Price: High-Low
          </DropdownItem>
          <DropdownItem
            onClick={() => handleSort({ key: 'price', type: 'asc' })}
            isActive={filters.sort?.key === 'price' && filters.sort.type === 'asc'}
          >
            Price: Low-High
          </DropdownItem>
          <DropdownItem
            onClick={() => handleSort({ key: 'title', type: 'asc' })}
            isActive={filters.sort?.key === 'title' && filters.sort.type === 'asc'}
          >
            Name: A-Z
          </DropdownItem>
          <DropdownItem
            onClick={() => handleSort({ key: 'title', type: 'desc' })}
            isActive={filters.sort?.key === 'title' && filters.sort.type === 'desc'}
          >
            Name: Z-A
          </DropdownItem>
          <DropdownItem
            onClick={() => handleSort({ key: 'rating', type: 'desc' })}
            isActive={filters.sort?.key === 'rating' && filters.sort.type === 'desc'}
          >
            Rate: High-Low
          </DropdownItem>
          <DropdownItem
            onClick={() => handleSort({ key: 'rating', type: 'asc' })}
            isActive={filters.sort?.key === 'rating' && filters.sort.type === 'asc'}
          >
            Rate: Low-High
          </DropdownItem>
        </Dropdown>
      ...
      <div className='flex gap-x-4'>
        {showFilters && <FiltersBar onFilter={setFilters} onClose={hideFilters} />}
        {isFetching ? (
          ...
          <>
            {products.length > 0 ? (
              ...
              <div className='w-full flex justify-center items-center'>Not found any products</div>
            )}
          </>
        )}
      </div>
