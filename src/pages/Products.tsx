...
const Products = () => {
  ...
  const [filters, setFilters] = useState<ProductsRequestParams>(parseSearchParams());
...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    showLoading();
    getProducts();
    const timeoutId = setTimeout(() => hideLoading(), 200);
    ...
    <div className='relative'>
      <div className='flex justify-end gap-x-4'>
        ...
        <Dropdown title='Sort By:' icon={<ArrowDownIcon className='size-6'/>}>
          <DropdownItem onClick={() => handleSort({key:'price', type: 'desc'})}>Price: High-Low</DropdownItem>
          <DropdownItem onClick={() => handleSort({key:'price', type: 'asc'})}>Price: Low-High</DropdownItem>
          <DropdownItem onClick={() => handleSort({key:'title', type: 'asc'})}>Name: Asc-Desc</DropdownItem>
          <DropdownItem onClick={() => handleSort({key:'title', type: 'desc'})}>Name: Desc-Asc</DropdownItem>
          <DropdownItem onClick={() => handleSort({key:'rating', type: 'desc'})}>Rate: High-Low</DropdownItem>
          <DropdownItem onClick={() => handleSort({key:'rating', type: 'desc'})}>Rate: Low-High</DropdownItem>
        </Dropdown>
      </div>
      {/* Show Products */}
          ...
          <div className='fixed overflow-scroll md:overflow-auto md:relative inset-0 w-full md:w-[200px] bg-white flex-shrink-0 rounded-sm p-6 md:p-2 z-10'>
            ...
            {categories && categories.map((category) => (
              <div className='w-full p-2 text-sm' key={category.id}>
                <Checkbox isChecked={selectedCategoryId === category.id} id={category.id.toString()} onChange={() => handleCheckboxChange('categoryId', category.id)}>{category.name.toUpperCase()}</Checkbox>
              </div>
            ))}
            <h5 className='font-medium text-base'>Price</h5>
            <div className='grid grid-cols-2 gap-x-2 w-full mt-2'>
              <input type="number" placeholder='Min' value={minPrice || undefined} className='p-2 outline-none border border-gray-300 text-xs appearance-none [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none' onChange={(e) => handleChangePrice('min', e)}/>
              <input type="number" placeholder='Max' value={maxPrice || undefined} className='p-2 outline-none border border-gray-300 text-xs appearance-none [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none' onChange={(e) => handleChangePrice('max', e)}/>
            </div>
            ...
            <div className='flex justify-end gap-x-4 mt-4'>
              <button className='text-sm px-3 py-1 border-[1px] border-black hover:bg-gray-200 rounded-md' onClick={handleClearFilters}>Clear</button>
              <button className='bg-gray-900 hover:bg-gray-700 px-3 py-1 text-white rounded-md' onClick={handleApplyFilters}>Apply</button>
            </div>
          </div>
        )}
        {
          products.length > 0 ? (
            <div className='w-full'>
              ...
              <div className='my-6 flex justify-center'>
                <Pagination current={filters.page} onChange={setFilters} total={totalPages}/>
              </div>
            </div>
          ) : (
